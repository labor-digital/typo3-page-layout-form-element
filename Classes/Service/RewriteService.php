...
class RewriteService implements PublicServiceInterface
{
    ...
    public function cleanUpPageModule(string $body, bool $forIframeRequest = false): string
    {
        $replacements = [
            ...
            // Preview button for content elements...
            '/<a[^>]*?class="[^"]*?t3js-editform-view.*?<\\/a>/si' => '',
            ...
            '/(<div class="[^"]*?module-docheader-bar-navigation.*?>).*?(<\\/div>\s+<div class="[^"]*?module-docheader-bar-buttons)/si' => '$1$2',
            // Remove everything above the content...
            '/(<form.*?>).*?(<div class="(?:form-inline|t3-grid-container|typo3-TCEforms))/si' => '$1$2',
